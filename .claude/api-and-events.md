...
```
`eventbus.Publisher` (injected by `postgres.TxRunner`) owns enqueue and takes `events.Codec` — `ValidatingCodec` wrapping `events.NoopCodec` (no local enqueue Codec). Missing schema is a pass-through (realm); `SchemaValidator` remains the fail-closed unit-test helper. `GlueCodec` pre-fetches each schema's latest version ID from Glue at startup; `events.NoopCodec` is the SNS pass-through when `GLUE_REGISTRY_NAME` is unset.
