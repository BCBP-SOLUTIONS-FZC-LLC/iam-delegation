...
Pulled from `IMPLEMENTATION_NOTES.md`'s DLG-D1..D20 as-built decision register — the ones most load-bearing for anyone touching this code:
...
4. **Dual review-warning bucket, not a single deadline (DLG-D6/D7/DLG-Q6)** — `review_last_warned_bucket` tracks whether the 7-day or 3-day notice already fired, independently. `ended_reason=review_expired` is a distinct value from `expired` so consumers can tell an auto-ended-on-review-lapse delegation from a plain `ends_at` expiry.
...
10. **Metrics are registered but not instrumented (DLG-D19, known gap)** — `internal/adapter/outbound/metrics.Metrics` exists and is registered with the Prometheus registerer at startup, but no service-layer call site invokes any of its methods. `iam_delegation_*` counters will read zero. This is a documented gap, not a bug — see Troubleshooting below.
...
- **New repository migration:** this repo ships a single migration pair so far, `000001_schema.{up,down}.sql` — the next one is `000002_description.{up,down}.sql`. See `CONTRIBUTING.md` for the full checklist (`CREATE INDEX CONCURRENTLY` guidance, RLS test requirements).
...
**`iam_delegation_*` metrics reading zero:** expected (DLG-D19) — the `Metrics` object is registered but no service-layer call site invokes a method on it yet. This is a known, documented gap, not something to debug as a wiring bug.
