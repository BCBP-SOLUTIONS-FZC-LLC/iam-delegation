...
16. **`errorLogger` logs unclassified 500s; classified `domain.Error`s do not (DLG-D36)** — `internal/adapter/inbound/http/errors.go`'s `HandleError` only logs through the shared gincommon logger when an error isn't a recognized `*domain.Error` — every mapped domain error (400/404/409/422/503 etc.) is expected, user-facing, and deliberately not logged as a server-side event. Don't add logging to the recognized-error branch; that would flood logs with routine 4xx traffic.
