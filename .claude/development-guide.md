...
Pulled from `ARCHITECTURE.md`'s DLG-D1..D39 decision register — the ones most load-bearing for anyone touching this code:
...
14. **Idempotency create-guard is a `SETNX` reservation, not a plain check-then-act (DLG-D35)** — `port.IdempotencyStore.Reserve`/`Release` close a real race the original `Get`-then-`Save` implementation left open (two concurrent same-key `POST /delegations` could both miss the `Get` and both insert). `Create` reserves the key before any side effect and releases it on failure; a losing concurrent caller gets `409 idempotency_key_in_flight`, not a duplicate row. Also: `SYSTEM_DATABASE_URL`'s fail-fast (DLG-D34) now triggers outside any `isDevLikeEnvironment` name, not only a literal `"production"` — don't reintroduce a bare `== "production"` check when touching `cmd/server/config.go`. **Updated in DLG-D37:** the environment name it checks is `resolveAppEnv()` (`APP_ENV` if set, else `ENVIRONMENT`), not a raw `os.Getenv("ENVIRONMENT")` read, and `test` is now a fourth recognized dev-like alias alongside `development`/`dev`/`local` — `cmd/reconciler` uses the identical helper.
15. **`outbox_events.payload` is `TEXT`, not `JSONB` (DLG-D38)** — a `pgx` SimpleProtocol connection (PgBouncer transaction pooling) sends `[]byte` values as bytea hex, not UTF-8 text; a `JSONB` column would silently store the hex-encoded wrapper instead of the JSON payload. `trg_outbox_normalize_payload` decodes that on INSERT. Don't change this column back to `JSONB` without re-verifying the PgBouncer path end-to-end.
16. **`errorLogger` logs unclassified 500s; classified `domain.Error`s do not (DLG-D36/D42)** — `internal/adapter/inbound/http/errors.go`'s `HandleError` only logs through the shared gincommon `port.Logger` when an error isn't a recognized `*domain.Error` — every mapped domain error (400/404/409/422/503 etc.) is expected, user-facing, and deliberately not logged as a server-side event. Don't add logging to the recognized-error branch; that would flood logs with routine 4xx traffic. Unclassified 500s include gincommon `trace_id`/`request_id` plus path/method. Don't reintroduce a per-package `Logger` interface — every adapter takes `internal/core/port.Logger` (Zap via `logger.NewLogger`); `postgres.NewLoggerAdapter` is the only adapter, for pgcommon's Field logger.
17. **SLO burn-rate alerts are a formalization, not a new policy (DLG-D39)** — `deploy/monitoring/slo-rules.yml` and `templates/prometheusrule.yaml`'s `iam_delegation_slo_*` groups restate targets §14.1/§14.5 already specify (99.9% write-path availability, DLG-D3's membership-check dependency, the DEL-6 reconciler-defer counters, the cascade DLQ) as multi-window burn-rate budgets. Unlike the sibling services with a standalone `slo-rules.yml`, this service's copy is also rendered by the Helm chart — a metric rename must be applied in all three of `deploy/monitoring/app-alerts.yml`, `deploy/monitoring/slo-rules.yml`, and `templates/prometheusrule.yaml`, not just one.
...
| `db_unavailable` | 503 | Postgres connection/network failure (`wrapConnErr` — SQLSTATE 08/53/57/58, `puddle.ErrClosedPool`, or a Go-level network/IO error like `io.EOF`/`*net.OpError`, DLG-D37) |

---

**Document version:** 1.3 (updated for DLG-D36 through DLG-D39 — logging/db/events re-audits, `resolveAppEnv()`/`test` alias, `db_unavailable` error code, SLO burn-rate alerting)
