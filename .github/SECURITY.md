...
- **Cascade consumer idempotency** — `processed_events` dedup prevents a redelivered `MembershipRevoked`/`TenantMembershipsPurged` from double-processing; a bypass could double-clear availability or emit duplicate `DelegationEnded` events.
