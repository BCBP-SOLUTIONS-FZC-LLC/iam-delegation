...
`postgres.TxRunner`) owns enqueue and reads the tx via `port.TxFromContext` (DLG-D44, matching
`iam-user-profile` / `iam-org-membership`); `ValidatingCodec` validates the payload against its registered
...
bytea-hex on INSERT — DLG-D38, matching `iam-user-profile` / `iam-org-membership`). Wire-format
(Glue) encoding happens later, transiently, at SNS-publish time only —
never written back to the outbox row. SNS publish, SQS consume, and the
outbox runner are constructed through `platform-events/pkg/config`
(`LoadSNS` / `LoadSQS` / `LoadOutbox` + `SNSConfigFromEnv` /
`SQSConfigFromEnv` / `SQSConsumerOptions` / `RunnerConfigFromEnv`, DLG-D44),
matching `iam-user-profile` / `iam-org-membership`. `CASCADE_*` overlays
`SQS_QUEUE_URL` / `SQS_CONCURRENCY` because this service cannot use those
library names as-is.
...
once in each binary's `main()` and threaded everywhere as a single `internal/core/port.Logger`
(matching gincommon's Debug/Info/Warn/Error(msg, fields) shape exactly, DLG-D42) —
`gincommon.Config.Logger`, `platform-events`' publisher/consumer/outbox `Logger` fields, inbound
HTTP/SQS adapters, outbound adapters, reconciler jobs, and (via
`internal/adapter/outbound/postgres.NewLoggerAdapter`) `platform-pgcommon`'s `domain.Logger` —
with no separate hand-rolled logger and no per-package Logger interface anywhere in the process
(DLG-D22, collapsed in DLG-D42). HTTP `http_request` lines come from `ObservabilityMiddlewares`'
LoggingMiddleware; inbound `HandleError` logs unclassified 500s through that same logger, including
gincommon `trace_id`/`request_id` plus path/method (DLG-D36/D42). Logs carry `delegation_id`,
...
- `cmd/server`: `GaugeRepository` — the 5-minute `iam_delegation_active_gauge{tenant}` snapshot
  exporter (`pgcommon.Pool.WithConn`, not `RunInTx` — DLG-D43, matching O&M exporters).
...
| DLG-D41 | **GAP-DEL-4 closed by code inspection — Glue cross-registry decode is safe.** `eventbus.GlueDecodeCodec.Decode` (line 193, `codec.go`) accepts the `schemaID string` parameter purely for `events.Codec` interface compatibility and immediately discards it (`_ string`). It delegates directly to `stripGlueHeader`, which performs two checks only: (1) payload length ≥ 18 bytes; (2) first byte == `0x03` (magic byte). The 16-byte schema version UUID at bytes 2:18 is **skipped without validation** — `encoded[glueHeaderSize:]` indexes past it. No Glue API call, no registry lookup, no cross-check against `GlueRegistryName` (`"iam-delegation-events"`). This means the 18-byte header emitted by User Profile's `iam-events` registry decodes identically to one emitted by Core's `iam-membership-events` registry — the UUID differs between registries but is never inspected. `platform-events` v1.4.0's SQS consumer calls `codec.Decode` only when the received envelope's `dataschema` field is non-empty (SchemaID set by the producer); when NoopCodec is used (environments with `GLUE_REGISTRY_NAME` unset), SchemaID is empty and `Decode` is never called at all. Verified against `platform-events@v1.3.0/internal/adapter/outbound/sqs/consumer.go dispatch()` (the v1.4.0 SQS consumer code is structurally identical). The subscription also requires `RawMessageDelivery=true` — corrected in `deploy/messaging/sns_subscriptions.tf.example` (was incorrectly `false`; `init-floci.sh` sets it correctly). |
| DLG-D40 | **Stale delegate pointer after cascade — accepted residual risk (Gap 7 / cross-service audit).** When `CascadeService.EndForUser` ends a delegation and the subsequent fail-open `ClearDelegatePointer` call to User Profile fails (network timeout, UP outage), the delegation row is correctly `ended` in Postgres but UP's `delegate_id` pointer on the delegator remains set until UP self-heals or the delegator updates their own availability. **Option A chosen:** the stale pointer is display-only — the delegation is correctly closed and Workflow Service receives `DelegationEnded`. The expiry cron does not re-attempt the clear on already-ended rows. This is an intentional continuation of the fail-open cancel pattern (`DLG-D10`/LLD §11.2) extended to the cascade path. Option B (write a `pending_pointer_clears` retry record processed by the expiry cron, recommended) and Option C (User Profile subscribes to `DelegationEnded` events, cleanest long-term) were considered and deferred as cross-team scope. Full option analysis: `Delegation/gap_docs/delegation-cross-service-compatibility-gaps.md` Gap 7. |
| DLG-D43 | **Sep-15 cross-service compatibility audit — three correctness fixes, one missing pre-flight.** (1) **Gap 3 — dedicated pointer-clear endpoint:** `UserProfileClient` gains a third method, `ClearDelegatePointer(ctx, tenantID, userID)`, mapping to `DELETE /api/v1/internal/users/:id/availability/delegate` on User Profile. Replaces the earlier `SetAvailability{ClearDelegate:true}` pattern which over-wrote the full availability state (status + timestamps) just to clear the delegate pointer — correctness gap when the delegator was concurrently OOO for a non-delegation reason. All paths that previously called `SetAvailability` to clear now call `ClearDelegatePointer` instead: Create's compensating clear on tx failure, Cancel (fail-open), Expiry cron (fail-open / defer-and-retry), and cascade `EndForUser` (fire-and-forget). `deploy/messaging/sns_subscriptions.tf.example` updated: the `delegation-cascade-q` `iam.user.events` subscription now sets `RawMessageDelivery=true` (was incorrectly `false`), matching the filter and consumer wiring DLG-D41 already documented as correct. (2) **GAP-DEL-2 — delegate OOO pre-flight in Create:** `GetAvailability(delegate)` is now called BEFORE `SetAvailability` in `DelegationService.Create`. This is the only call site with `starts_at` context — User Profile's own `SetAvailability` cannot perform this check. Logic: for an immediate delegation, blocks if snap.Status=="ooo"; for a future-dated (scheduled) delegation, blocks only if snap.OOOUntil != nil && ooo_until.After(starts_at) — a delegate currently OOO who clears before `starts_at` is not blocked. Resolves to `422 delegate_unavailable`. (3) **Open-ended delegation OOO window clamping (DLG-D29 companion):** For delegations without `ends_at`, `SetAvailability` now sends `review_due_at` as `ooo_until` instead of `nil`. Without this, UP's `ValidateOOOWindow` rejected every open-ended create with 422 (`ooo_until` is required by UP whenever `status="ooo"`). The `review_due_at` is always within UP's 180-day cap (review_window_days is policy-bounded to 1..180). The identical fix is also applied in `delegation_activation.go` (DLG-D29 confirmed). (4) **Extend resync:** `DelegationService.Extend` now calls `SetAvailability` fail-open after every successful `ExtendReview`, re-syncing UP's `ooo_until` to the new `review_due_at`. Without this, UP's own OOO-expiry sweep reset the delegator to available once the original `review_due_at` lapsed, even though the delegation was still active. |
| DLG-D42 | **Local dev AWS emulator migrated from LocalStack to Floci.** `docker-compose.yml` replaces `localstack/localstack:4.4.0` with `floci/floci:2.1.0-compat` (MIT, always-free). Key difference: Floci includes Glue Schema Registry in its free tier — `LocalStack Community` gates it behind a Pro subscription. Consequences: (1) `docker-compose.pro.yml` deleted — no longer needed; (2) `scripts/init-localstack.sh` replaced by `scripts/init-floci.sh` using the standard `aws` CLI instead of `awslocal`; (3) `GLUE_REGISTRY_NAME`/`GLUE_REGISTRY_ARN` now set in `docker-compose.yml` by default so `GlueCodec` (real 18-byte wire format) runs locally without any opt-in — `NoopCodec` is no longer the local dev default; (4) AWS test credentials changed from `localstack`/`localstack` to `test`/`test`; (5) `floci-ui` web console added at host port 4501 (`floci/floci-ui:0.5.0`); (6) healthcheck now waits for the last Glue schema (`DelegationEscalationRequested`) so the app's `depends_on` isn't satisfied until the full registry is provisioned. `init-floci.sh` also correctly provisions the fourth downstream subscriber queue (`delegation-events-user-profile-q`, `DelegationEnded` filter) that `init-localstack.sh` was missing. |
...
| DLG-D40 | **Production-readiness sweep**, prompted by a direct "is this production ready" review with a follow-up "fix all." `registerDocsRoutes` (`internal/adapter/inbound/http/router.go`) only puts `docsAuthMiddleware` in front of `/swagger` and `/asyncapi` when `Environment=="production"` AND `AuthToken!=""` — `loadConfig` had no corresponding fail-fast, so `DOCS_ENABLED=true` with `DOCS_AUTH_TOKEN` left unset in production would silently serve both docs surfaces with no auth check at all. Closed with a new `loadConfig` fail-fast (LLD §15) and a regression test (`TestLoadConfig_DocsAuthTokenFailFast`). Also removed `Metrics.SetActiveGauge`, a public method with no call site outside its own unit test (`ReplaceActiveGauges`, wired to the active-gauge exporter, already sets the same gauge inline) — implementation-hardening only, no LLD-visible content. Rest of the sweep (full `make ci`, real merged coverage at 99.0%, RLS/config fail-fast re-verification, IAM-policy/secrets spot-check) found no other code-level gaps; DLG-D26's `iam.user.events` subscription and DLG-D13's `TenderScopeClient` wiring remain open cross-team/infra items, unchanged by this pass. |
| DLG-D41 | **§17.4 E2E suite implemented** (LLD rev 2.18), closing a gap the DLG-D40 sweep surfaced but didn't fix: `make test-e2e` (`-tags=e2e`) silently re-ran the same untagged unit suite as `make test-unit`, since no file anywhere in the repo declared `//go:build e2e` — despite `.github/workflows/validate-test.yml` already calling it as a required CI step (that wiring was already correct; only the test content was missing). New `cmd/server/e2e_test.go` boots the real composition root against real Postgres/Valkey/floci (SNS+SQS-compatible) containers — not `miniredis`/fakes, which is what the unit suite already covers — with User Profile/Org Membership faked via local `httptest.Server`s. Required one refactor: `run(logger Logger) error` → `run(ctx context.Context, logger Logger) error`, so a test can cancel its own context to drive the exact graceful-shutdown path deterministically instead of only via `main()`'s OS-signal `NotifyContext`; `main()` passes `context.Background()`, identical behavior. Four subtests cover §17.4 verbatim (create/cancel fan-out, expiry, review daily cascade, user-removal cascade incl. the DLG-EVT-4 silent delegator-side end). `make test-unit`/`test-ci` (no `-tags=e2e`) confirmed unaffected — coverage gate unchanged at 99.0%. |
| DLG-D42 | Logs/metrics/traces re-checked against `iam-user-profile` / `iam-org-membership`. Init path was already gincommon-native (`logger.NewLogger` → `InitTracingFromEnv` → `ObservabilityMiddlewares` → `metrics.Register` onto `MetricsRegisterer` → `events`/`pgmetrics` `InitWithRegisterer` → dedicated `:9090/metrics` → `otelhttp` outbound + `PropagateHeaders`, DLG-D36). Remaining sibling gap: every adapter/jobs package still declared its own duck-typed `Logger` instead of one `internal/core/port.Logger` like UP/O&M. Collapsed onto that single port (Zap-backed via `logger.NewLogger`; `postgres.NewLoggerAdapter` remains the only adapter, for pgcommon's Field logger). The reconciler now also calls `events`/`pgmetrics` `InitWithRegisterer` after `ObservabilityMiddlewares`. Unhandled HTTP 500s include gincommon `trace_id`/`request_id` plus path/method. `SlogStyleLogger` is not copied — this service has no slog-style call sites. |
...
- **Stale delegate pointer after cascade ClearDelegatePointer failure (DLG-D40):** when
  `EndForUser` (MembershipRevoked) ends a delegation and the subsequent fail-open
  `ClearDelegatePointer` call to User Profile fails (UP is down or times out), the delegation
  row is correctly `ended` in Postgres but User Profile's `delegate_id` pointer remains set.
  User Profile shows the delegator as still having a delegate pointer that points at a user
  who has left the tenant. The expiry cron does not re-attempt this clear (it only processes
  `active` rows). **Accepted behaviour (DLG-D40, Option A):** the stale pointer is display-only
  — the delegation is correctly ended and Workflow Service receives `DelegationEnded`. The
  delegator can clear it by updating their own availability. Option B (retry record) and
  Option C (UP subscribes to `DelegationEnded`) were considered and deferred — see
  `Delegation/gap_docs/delegation-cross-service-compatibility-gaps.md` Gap 7 for the full
  option analysis.
