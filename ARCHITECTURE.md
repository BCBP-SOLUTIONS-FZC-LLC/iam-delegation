...
`iam.user.events` filtered to `EventType = "user.updated"`, feeding the existing
...
delegation data itself is never affected, only the idempotency guarantee's strength. There is no
...
rows under RLS (fail-quiet, not fail-loud); **in production, both binaries now fail fast at startup
if `SYSTEM_DATABASE_URL` is unset when `ENVIRONMENT=production`** (DLG-D34), rather than silently
degrading every cross-tenant sweep query to zero rows with no alert.
