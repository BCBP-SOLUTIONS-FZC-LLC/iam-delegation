...
| DLG-D25 | **Cross-service bug fix, confirmed against `iam-user-profile`'s actual runtime behavior, not just this service's own code.** A delegation created with a future `starts_at` immediately called User Profile's `PUT /internal/users/:id/availability {status: ooo}` and emitted `DelegationStarted` — both before the leave actually began, showing the delegator as OOO and routing Workflow work to the delegate early. Root cause was on both sides: User Profile applied whatever `status` a caller sent regardless of `ooo_from`, and this service treated `starts_at` as purely informational once membership checks passed. Fixed here by adding a `scheduled` value to `delegation_status` (folded into `000001_schema` per this repo's still-undeployed convention) between the implicit initial state and `active`: `Create` now computes `isScheduled := starts.After(now)` and, when true, skips the User Profile call and the `DelegationStarted` enqueue entirely, inserting the row as `scheduled` instead of `active` — no compensating `ClearDelegate` call either, since User Profile was never touched. A new `delegation-activation` CronJob (`cmd/reconciler/jobs/delegation_activation.go`, `*/5 * * * *`, mirrors `delegation-expiry`'s structure) lists rows via the new `ListScheduledBefore` query, calls User Profile's `SetAvailability`, and on success calls the new `Activate` repository method (`UPDATE ... SET status='active' WHERE status='scheduled' AND record_version=$expected`, optimistic-lock-guarded) inside the same transaction as the `DelegationStarted` enqueue; a User-Profile failure increments `iam_delegation_activation_deferred_total` (mirroring `RecordExpiryDeferred`) and retries next tick rather than failing the whole batch. `End`/`probeVersionConflict`/`EndForUser` (the `MembershipRevoked` cascade) were broadened to treat `scheduled` as non-terminal alongside `active`, so a scheduled delegation for a member who leaves the tenant before `starts_at` is cancelled rather than left to activate against a departed member. `port.DelegationRepository` gained `Activate`/`ListScheduledBefore` directly (only one fake implements this interface, unlike `iam-user-profile`'s `AvailabilityRepository`, so no test-fake ripple to avoid). `starts_at` within `skewTolerance` (5s) of `now` — including the default no-`starts_at` case — is never treated as scheduled, so normal immediate creates are unaffected. See `iam-user-profile`'s `CHANGELOG.md`/`ARCHITECTURE.md` for the matching server-side deferred-activation fix (that service defers the status flip independently of what any caller, including this one, requests). Deliberately **not** added in this pass: an HTTP on-demand-trigger endpoint for Activation analogous to DLG-I1/I2 (`/internal/delegations/expire`/`/review-sweep`) — the 5-minute cron cadence was judged sufficient for a background reconciliation job, and adding one would require new DTOs/handler/route/swagger annotations with no corresponding LLD requirement. |
| DLG-D26 | **Cross-service bug fix (Bug 2), confirmed against `iam-user-profile`'s actual runtime behavior.** Disabling a delegate never ended their active delegations — this service's only inbound cascade signal was Core's `MembershipRevoked` (a user leaving the tenant entirely), and "disabled" is a status flip within the tenant, not a departure; a disabled delegate's grants stayed `active` indefinitely, still routing Workflow work to someone who could no longer act on it. Root cause on the User Profile side (separately fixed there): `sweepInboundDelegatesTx` correctly cleared the delegate pointer on disable, but built the resulting `UserAvailabilityChanged` event from a synthetic `{status: available}` object instead of the delegator's real row — and `UserUpdatedPayload` had no field carrying the new status value, so no consumer could even react to "disabled" specifically. Fixed on this side by treating `delegation-cascade-q` as carrying a **second** SNS subscription (in addition to Core's `iam.membership.events`) onto User Profile's `iam.user.events`, filtered to `EventType = "user.updated"` — provisioned externally, like every other queue subscription this repo doesn't own infrastructure for. `CascadeConsumer.Handle` decodes each `UserUpdated` payload and dispatches only when `status == "disabled"` (most deliveries are unrelated field changes the filter policy can't exclude, since it filters on `EventType` only); `CascadeService.EndForDisabledDelegate` ends every active/scheduled delegation where the disabled user is the delegate and emits `DelegationEnded{ended_reason: delegate_disabled}` — a new `EndReason` value, distinct from `delegate_removed` (which remains `MembershipRevoked`-only). Unlike `EndForUser`, the new `EndForDisabledDelegate` repository method deliberately does **not** set `deleted_at`: the user is still a tenant member, so the row stays a normal historical record (matching regular `End`/`Cancel` semantics), not a scrub. No User Profile pointer-clear call is made from this side: User Profile's own disable flow already clears the delegate pointer atomically, inside the same transaction that publishes the very `UserUpdated` event this consumer reacts to, so by the time it's seen, User Profile's side is guaranteed done. See `iam-user-profile`'s `CHANGELOG.md`/LLD revision for the `UserUpdatedPayload.status` field addition (additive, `additionalProperties: true`) and the `ClearInboundDelegates` return-type fix (`[]uuid.UUID` → `[]*domain.Availability`) this depends on. |
