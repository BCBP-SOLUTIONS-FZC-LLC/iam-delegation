...
│   │       ├── catalogadmin/          # CatalogAdminClient HTTP impl (GAP-020) — scope=department scope_id validation via CAT-7
...
│   └── eventschema/                   # embedded JSON Schemas — 4 published + 3 consumed, generated from api/asyncapi.yaml (make extract-schemas, DLG-D51)
├── pkg/requestctx/                    # gateway-identity / tenant-actor extraction helpers
├── api/
│   ├── asyncapi.yaml                  # AsyncAPI 3.0 — IAM Delegation Events: 4 published + 3 consumed event types; governance annotations (x-lifecycle, x-owner, x-forward-compatibility, x-semantic-contract, x-consumer-sdk, x-version-governance, x-usage-override)
│   └── embed.go                       # //go:embed asyncapi.yaml
├── docs/
│   ├── lld/                           # LLD, current rev 2.20 — design-time source of truth; as-built decisions DLG-D48/D49 in ARCHITECTURE.md
│   ├── architecture/                  # index + 13 Mermaid diagrams
│   ├── swagger/                       # generated by `make swag` + `scripts/patch-swagger-extensions.py` — not hand-authored
...
│   └── patch-swagger-extensions.py   # post-processes docs/swagger/ after make swag — injects tags block for Swagger UI grouping
...
The schema-version UUID is resolved **once, at startup, by definition**: `GlueCodec` calls
`GetSchemaByDefinition` with each schema exactly as this binary embeds it
(`internal/eventschema/`), so every event carries the version that describes the payload this
binary produces — not whatever is latest in the registry. There is no background refresh. If a
definition isn't registered (a deploy that outran `schema-registry.yml`), startup fails and the pod
CrashLoops until registration lands; `make schema-verify` runs the same check pre-deploy. CI and
`make schema-register` register a PascalCase-renamed copy of the snake_case files
(`.github/scripts/stage-produced-event-schemas.sh`), because `schema-gov register` names each schema
after its file stem.
...
Every payload is validated against its consumed schema (`internal/eventschema/`, generated from
`api/asyncapi.yaml`) before dispatch. The schemas require what the producers require and nothing
more, so a valid upstream event is never rejected. A payload that violates it is never acked or
marked processed — it is retried and lands in the DLQ for inspection, so a malformed event can't
end the wrong delegations (Go's UUID parser, for one, is more lenient than the contract).
