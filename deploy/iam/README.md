...
| `glue:GetSchemaByDefinition` (+ read-only siblings) | `GlueCodec` resolves each schema's version ID once at startup by this binary's embedded definition (no refresh) | §10.3.1 |
