...
The O&M `DELEGATION_REVIEW_WINDOW_DAYS` env var is gone entirely — the window is per-tenant in `delegation_tenant_settings`, defaulting to `policyDefaults` only when a tenant has no row. No cross-service config URL or TTL exists any more (DLG-Q2). Secrets via the platform store; base URLs required (fail-fast on empty for the data-bearing clients). `SYSTEM_DATABASE_URL` (the BYPASSRLS pool backing DLG-I3/I4, the four CronJobs' cross-tenant sweeps, and the active-gauge exporter) is required in Helm always, and both binaries now fail fast at startup if it is unset when `ENVIRONMENT=production` (DLG-D34) — outside production it falls back to the app pool's DSN with a startup warning, degrading cross-tenant reads to RLS-filtered zero rows rather than erroring.
