...
import (
	...
	apispec "github.com/BCBP-SOLUTIONS-FZC-LLC/iam-delegation/api"
)
...
// probes may otherwise turn into a hot path. Ported verbatim from
// iam-tender-acl's identical viewer (itself ported from iam-user-profile's
// original) — the renderer is entirely YAML-driven, no service-specific
// name lists, so it needed no logic changes to work against this service's
// own api/asyncapi.yaml.
var (
...
// event contract (api/asyncapi.yaml).
//
// @Summary   AsyncAPI catalog viewer
// @Tags      infra
// @Produce   html
...
// the rendered catalog page at /asyncapi.
//
// @Summary   Raw AsyncAPI spec
// @Tags      infra
// @Produce   application/yaml
// @Success   200  {string}  string  "AsyncAPI YAML"
...
`)
  ...
  <div>
    <h1>%s</h1>
    <p>Events on iam.delegation.events + the delegation-cascade-q consumer contract</p>
  </div>
...
// message's bindings node, if present.
